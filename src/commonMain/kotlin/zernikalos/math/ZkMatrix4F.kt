...
@JsExport
@Serializable
class ZkMatrix4F: ZkAlgebraObject {
    ...
    operator fun plus(m: ZkMatrix4F): ZkMatrix4F {
        val result = ZkMatrix4F()
        add(result, this, m)
        return result
    }

    operator fun minus(m: ZkMatrix4F): ZkMatrix4F {
        val result = ZkMatrix4F()
        subtract(result, this, m)
        return result
    }

    operator fun times(m: ZkMatrix4F): ZkMatrix4F {
        val result = ZkMatrix4F()
        mult(result, this, m)
        return result
    }
    ...
    fun translate(translation: ZkVector3F) {
    ...
    companion object {
        ...
        val Identity: ZkMatrix4F
            get() = ZkMatrix4F()

        fun identity(result: ZkMatrix4F) {
        ...
        fun add(result: ZkMatrix4F, m1: ZkMatrix4F, m2: ZkMatrix4F) {
            for (i in 0..15) {
                result.values[i] = m1.values[i] + m2.values[i]
            }
        }

        fun subtract(result: ZkMatrix4F, m1: ZkMatrix4F, m2: ZkMatrix4F) {
            for (i in 0..15) {
                result.values[i] = m1.values[i] - m2.values[i]
            }
        }

        fun mult(result: ZkMatrix4F, lm: ZkMatrix4F, rm: ZkMatrix4F) {
            for (i in 0 .. 3) {
        ...
        @JsName("multVec4")
        fun mult(result: ZkVector4F, m: ZkMatrix4F, v: ZkVector4F) {
            val x = v.x
            val y = v.y
            val z = v.z
            val w = v.w
        ...
        @JsName("multVec3")
        fun mult(result: ZkVector3F, m: ZkMatrix4F, v: ZkVector3F) {
            val x = v.x
            val y = v.y
            val z = v.z
            val w = 1.0f
        ...
        fun transpose(result: ZkMatrix4F, m: ZkMatrix4F) {
            for (i in 0..3) {
                val k = i * 4
        ...
        fun transposeIp(result: ZkMatrix4F) {
        ...
        fun translate(result: ZkMatrix4F, m: ZkMatrix4F, translation: ZkVector3F) {
            for (i in 0..11) {
        ...
        @JsName("translateByVector")
        fun translate(result: ZkMatrix4F, translation: ZkVector3F) {
            innerTranslate(result, translation.x, translation.y, translation.z)
        }

        fun translate(result: ZkMatrix4F, x: Float, y: Float, z: Float) {
            innerTranslate(result, x, y, z)
        }

        private fun innerTranslate(result: ZkMatrix4F, x: Float, y: Float, z: Float) {
            for (i in 0..3) {
                result.values[12 + i] = result.values[i] * x + result.values[4 + i] * y + result.values[8 + i] * z + result.values[12 + i]
            }
        }

        fun rotate(result: ZkMatrix4F, q: ZkQuaternion) {
            val rotQuat = ZkMatrix4F()
            fromQuaternion(rotQuat, q)
            val aux = ZkMatrix4F(result.values)
            mult(result, aux, rotQuat)
        }

        @JsName("scaleByVectorCopy")
        fun scale(result: ZkMatrix4F, m: ZkMatrix4F, s: ZkVector3F) {
            for (i in 0..3) {
                result[i] = m[i] * s.x
        ...
        fun scale(result: ZkMatrix4F, s: ZkVector3F) {
            scale(result, result, s)
        }

        fun invert(result: ZkMatrix4F, m: ZkMatrix4F): Boolean {
            val src0  = m.values[0]
            val src4  = m.values[1]
            val src8  = m.values[2]
        ...
        fun lookAt(result: ZkMatrix4F, eye: ZkVector3F, center: ZkVector3F, up: ZkVector3F) {
            val f = ZkVector3F()

            // See the OpenGL GLUT documentation for gluLookAt for a description
            // of the algorithm. We implement it in a straightforward way:
            ZkVector3F.subtract(f, center, eye)
            ...
            val negEye = ZkVector3F()
            ZkVector3F.multScalar(negEye, -1.0f, eye)
            translate(result, negEye)
        }

        fun fromQuaternion(result: ZkMatrix4F, q: ZkQuaternion) {
            identity(result)
            val x: Float = q.x
            val y: Float = q.y
            val z: Float = q.z
