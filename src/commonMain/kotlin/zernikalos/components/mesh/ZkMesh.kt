package zernikalos.components.mesh

import kotlinx.serialization.Serializable
import kotlinx.serialization.Transient
import kotlinx.serialization.protobuf.ProtoNumber
import zernikalos.DrawModes
import zernikalos.Types
import zernikalos.ZkRenderingContext
import zernikalos.components.ZkComponent
import zernikalos.components.buffer.ZkBuffer
import zernikalos.components.buffer.ZkVertexArray

@Serializable
class ZkMesh: ZkComponent() {
    @ProtoNumber(1)
    private lateinit var attributeKeys: Map<String, ZkAttributeKey>
    @ProtoNumber(2)
    private lateinit var indices: ZkBuffer
    @ProtoNumber(3)
    private lateinit var vertices: Map<String, ZkBuffer>

    @Transient
    val vao: ZkVertexArray = ZkVertexArray()

    val useIndexBuffer: Boolean
        get() = indices.hasData

    override fun initialize(ctx: ZkRenderingContext) {
        vao.initialize(ctx)

        attributeKeys.forEach { (name, attr) ->
            val buffer = vertices[name]
            buffer?.initialize(ctx)
            attr.initialize(ctx)
        }

        if (useIndexBuffer) {
            indices.initialize(ctx)
        }
    }

    override fun render(ctx: ZkRenderingContext) {
        vao.render(ctx)
        if (useIndexBuffer) {
            ctx.drawElements(DrawModes.TRIANGLES.value, indices.count, Types.UNSIGNED_SHORT.value, 0)
        } else {
            // TODO: Fix this
            ctx.drawArrays(DrawModes.TRIANGLES.value, 0, vertices["position"]?.count!!)
        }
    }

}
