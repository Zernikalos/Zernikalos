...
class NpmPublisher:
    ...
    def __init__(self):
        self.github_user = os.environ.get('GITHUB_USER', 'Zernikalos')
    ...
    def get_github_credentials(self) -> bool:
        ...
        if not self.github_token:
            self.print_status("GitHub credentials required for .npmrc generation")
    ...
    def generate_npmrc(self) -> bool:
        ...
        try:
            cmd = [
                './gradlew', 'generateNpmrc',
                f'-Puser={self.github_user}',
                f'-Paccess_token={self.github_token}'
            ]
            
            result = subprocess.run(cmd, check=True, capture_output=True, text=True)
            self.print_success(".npmrc file generated successfully")
            return True
            
        except subprocess.CalledProcessError as e:
            self.print_error(f"Failed to generate .npmrc file: {e}")
            return False
    ...
    def build_packages(self) -> bool:
        ...
        try:
            result = subprocess.run(['./gradlew', 'jsBrowserProductionWebpack'], 
                                  check=True, capture_output=True, text=True)
            self.print_success("Packages built successfully")
            return True
            
        except subprocess.CalledProcessError as e:
            self.print_error(f"Failed to build packages: {e}")
            return False
