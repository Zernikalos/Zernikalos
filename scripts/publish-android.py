...
class AndroidPublisher:
    ...
    def get_github_credentials(self) -> bool:
        ...
        if not self.github_token:
            self.print_status("GitHub credentials required for Maven repository publishing")
    ...
    def build_project(self) -> bool:
        ...
        try:
            ...
            self.print_error(f"Failed to build project: {e}")
            return False
    ...
    def publish_android_debug(self) -> bool:
        ...
        try:
            ...
            self.print_error(f"Failed to publish Android Debug artifacts: {e}")
            if e.stderr:
    ...
    def publish_android_release(self) -> bool:
        ...
        try:
            cmd = [
                './gradlew', 'publishAndroidReleasePublicationToMavenRepository',
                f'-Puser={self.github_user}',
                f'-Paccess_token={self.github_token}'
            ]
            ...
            self.print_error(f"Failed to publish Android Release artifacts: {e}")
            if e.stderr:
                print(f"Error details: {e.stderr}")
            return False
    ...
    def publish_all_publications(self) -> bool:
        ...
        try:
            ...
            self.print_error(f"Failed to publish all publications: {e}")
            if e.stdout:
