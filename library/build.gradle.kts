...
kotlin {
    ...
    sourceSets {
        ...
        val oglMain by creating {
            kotlin.srcDir("library/src/oglMain/kotlin")
            dependsOn(commonMain.get())
        }

        val metalMain by creating {
            kotlin.srcDir("library/src/metalMain/kotlin")
            dependsOn(commonMain.get())
        }

        val webgpuMain by creating {
            kotlin.srcDir("library/src/webgpuMain/kotlin")
            dependsOn(commonMain.get())
        }

        androidMain {
            kotlin.srcDir("library/src/androidMain/kotlin")
            dependsOn(oglMain)
        }

        jsMain {
            // Required for compatibility with zdebugger (see webpack file)
            kotlin.srcDir("library/src/jsMain/kotlin")
            dependencies {
        ...
        val macosArm64Main by getting {
            dependsOn(metalMain)
            kotlin.srcDir("library/src/macosMain/kotlin")
        }

        iosMain {
            dependsOn(metalMain)
            kotlin.srcDir("library/src/iosMain/kotlin")
        }
